...
CREATE TABLE QUESITO_RISPOSTA_CHIUSA (
    ...
    FOREIGN KEY (TITOLO_TEST) REFERENCES TEST(TITOLO),
    FOREIGN KEY(ID) REFERENCES SEQUENZA_ID_QUESITI(ID_QUESITO),
...
DELIMITER $
CREATE PROCEDURE INSERIMENTO_QUESITO_CODICE(IN TITOLO_TEST VARCHAR(20), IN LIVELLO VARCHAR(20), DESCRIZ VARCHAR(60), NUM_RISPOSTE INT)
	...
	SET ID = (SELECT ID_QUESITO + 1 FROM SEQUENZA_ID_QUESITI WHERE SEQUENZA_ID_QUESITI.TITOLO_TEST = TITOLO_TEST)
	WHERE TITOLO_TEST = TITOLO_TEST AND ID = 0;
$
...
DELIMITER //
CREATE TRIGGER AGGIUNTA_SEQUENZA
...
BEGIN
    ...
    WHERE TITOLO_TEST = (SELECT TITOLO_TEST FROM TEST WHERE TITOLO_TEST = NEW.TITOLO_TEST);
END;
//
