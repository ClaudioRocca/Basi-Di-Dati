...
SELECT S.CODICE AS Codice_Studente,
       100 * (SUM(CASE WHEN RC.ESITO = TRUE THEN 1 ELSE 0 END) + SUM(CASE WHEN RQ.ESITO = TRUE THEN 1 ELSE 0 END))/(COUNT(RQ.ID_QUESITO) + COUNT(RC.ID_QUESITO))  AS Risposte_Corrette
FROM STUDENTE S
        JOIN RISPOSTA_QUESITO_CHIUSO RQ ON S.MAIL = RQ.MAIL_STUDENTE
		JOIN RISPOSTA_QUESITO_CODICE RC ON S.MAIL = RC.MAIL_STUDENTE
GROUP BY S.CODICE
