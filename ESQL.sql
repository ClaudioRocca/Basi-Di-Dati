...
CREATE TABLE TEST (
    ...
    VISUALIZZA_RISPOSTE BOOLEAN DEFAULT 0, -- oppure FALSE
    PRIMARY KEY (TITOLO),
...
SELECT CODICE, (100*SUM(CASE WHEN ESITO=TRUE THEN 1 ELSE 0 END)/ COUNT(*)) AS RISPOSTE_CORRETTE
FROM VISTA_RISPOSTE
GROUP BY CODICE
ORDER BY CODICE DESC;
$
...
-- DELIMITER $ -- FUNZIONA MA Errata perchè il campo visualizza risposte va settato di default a 0.
...
DELIMITER $ 
CREATE PROCEDURE INSERIMENTO_NUOVO_TEST(IN TITOLO VARCHAR(20), IN DATA_CREAZIONE DATE, IN FOTO BLOB, IN MAIL_DOCENTE VARCHAR(20))
BEGIN
...
DELIMITER $
CREATE PROCEDURE INSERIMENTO_QUESITO_CODICE(IN TITOLO_TEST VARCHAR(20), IN LIVELLO VARCHAR(20), IN DESCRIZIONE VARCHAR(60), IN NUM_RISPOSTE INT)
BEGIN
	INSERT INTO QUESITO_CODICE (TITOLO_TEST, LIVELLO, ID, DESCRIZIONE, NUM_RISPOSTE)
	VALUES (TITOLO_TEST, LIVELLO, 0, DESCRIZIONE, NUM_RISPOSTE);
...
DELIMITER $
CREATE PROCEDURE INSERIMENTO_QUESITO_RISPOSTA_CHIUSA(IN TITOLO_TEST VARCHAR(20), IN LIVELLO VARCHAR(20), IN DESCRIZIONE VARCHAR(60), IN NUM_RISPOSTE INT)
BEGIN
	INSERT INTO QUESITO_RISPOSTA_CHIUSA (TITOLO_TEST, LIVELLO, ID, DESCRIZIONE, NUM_RISPOSTE)
	VALUES (TITOLO_TEST, LIVELLO, 0, DESCRIZIONE, NUM_RISPOSTE);
...
SELECT COUNT(*) INTO num_quesiti FROM QUESITO_RISPOSTA_CHIUSA, QUESITO_CODICE WHERE QUESITO_RISPOSTA_CHIUSA.TITOLO_TEST = QUESITO_CODICE.TITOLO_TEST AND QUESITO_RISPOSTA_CHIUSA.TITOLO_TEST = NEW.TITOLO_TEST;
